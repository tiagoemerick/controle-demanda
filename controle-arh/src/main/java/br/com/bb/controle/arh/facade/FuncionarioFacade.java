...
public class FuncionarioFacade extends AbstractUtil {
	...
	public List<Funcionario> buscarFuncionariosPorInventario(Inventario inventario) throws Exception {
		if (inventario == null || inventario.getId() == null || inventario.getId().compareTo(0) == 0) {
			throw new IllegalArgumentException("Inventário inválido.");
		}
		List<Funcionario> funcionarios = new ArrayList<Funcionario>();
		try {
			funcionarios = funcionarioDao.findFuncionariosByInventarioId(inventario.getId());
		} catch (Exception e) {
			e.printStackTrace();
			throw new Exception("Erro ao buscar funcionários por inventário. Tente novamente.", e);
		}
		return funcionarios;
	}
